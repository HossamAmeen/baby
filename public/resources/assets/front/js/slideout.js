...
function Slideout(options) {
  ...
  // Sets paneel
  this.paneel = options.paneel;
  this.menu = options.menu;

  // Sets  classnames
  if (!this.paneel.classList.contains('slideout-paneel')) {
    this.paneel.classList.add('slideout-paneel');
  }
  if (!this.paneel.classList.contains('slideout-paneel-' + this._side)) {
    this.paneel.classList.add('slideout-paneel-' + this._side);
  }
...
Slideout.prototype.open = function() {
  ...
  this._opened = true;
  setTimeout(function() {
    self.paneel.style.transition = self.paneel.style['-webkit-transition'] = '';
    self.emit('open');
  }, this._duration + 50);
  return this;
};
...
Slideout.prototype.close = function() {
  ...
  setTimeout(function() {
    ...
    self.paneel.style.transition = self.paneel.style['-webkit-transition'] = self.paneel.style[prefix + 'transform'] = self.paneel.style.transform = '';
    self.emit('close');
  }, this._duration + 50);
  return this;
};
...
/**
 * Translates paneel and updates currentOffset with a given X point
 */
Slideout.prototype._translateXTo = function(translateX) {
  this._currentOffsetX = translateX;
  this.paneel.style[prefix + 'transform'] = this.paneel.style.transform = 'translateX(' + translateX + 'px)';
  return this;
};
...
Slideout.prototype._setTransition = function() {
  this.paneel.style[prefix + 'transition'] = this.paneel.style.transition = prefix + 'transform ' + this._duration + 'ms ' + this._fx;
  return this;
};
...
Slideout.prototype._initTouchEvents = function() {
  ...
  this.paneel.addEventListener(touch.start, this._resetTouchFn);
  ...
  this.paneel.addEventListener('touchcancel', this._onTouchCancelFn);
  ...
  this.paneel.addEventListener(touch.end, this._onTouchEndFn);

  /**
   * Translates paneel on touchmove
   */
  this._onTouchMoveFn = function(eve) {
    ...
    if (Math.abs(dif_x) > 20) {
      ...
      self.paneel.style[prefix + 'transform'] = self.paneel.style.transform = 'translateX(' + translateX + 'px)';
  ...
  this.paneel.addEventListener(touch.move, this._onTouchMoveFn);
...
Slideout.prototype.destroy = function() {
  ...
  this.paneel.removeEventListener(touch.start, this._resetTouchFn);
  this.paneel.removeEventListener('touchcancel', this._onTouchCancelFn);
  this.paneel.removeEventListener(touch.end, this._onTouchEndFn);
  this.paneel.removeEventListener(touch.move, this._onTouchMoveFn);
